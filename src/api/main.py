...
class FinanceApi:
    ...
    def make_request(
        self, request: schemas.RequestBase, interval_key: Optional[str] = None, **kwargs
    ) -> pd.DataFrame:
        ...
        output = self._download(
            tickers=tickers, period=period, interval=interval_key, **kwargs
        )
    ...
    def _download(self, tickers, **kwargs):
        ...
        since_last = (
            (datetime.now() - self.last_request).total_seconds()
...
class YahooApi:
    ...
    def get_stock_history(self, request: schemas.RequestBase):
        ...
        symbols = (
            request.stock if isinstance(request.stock, str) else ",".join(request.stock)
        )
