...
def create_daily(row: pd.Series, ticker: str, as_dict: bool = True):
    ...
    daily = schemas.DailyBase(stock_id=ticker, timestamp=row.name, open=row['Open'], high=row['High'], low=row['Low'], close=row['Close'], adj_close=row['Adj Close'], volume=row['Volume'])
