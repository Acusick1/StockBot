...
class Broker(ABC):
    ...
    def _update_trade_in_csv(self, trade: Trade):
        ...
        with open(self.trades_csv_file, mode='r') as trades_file, open(temp_file, mode='w', newline='') as f:
            
            reader = csv.DictReader(trades_file)
            writer = csv.DictWriter(f, fieldnames=reader.fieldnames)
            writer.writeheader()
            for row in reader:
                if row["ticker"] == trade.ticker and row["open_price"] == str(trade.open_price) and row["close_price"] == "":
...
class TestBroker(Broker):
    ...
    def _buy(self, ticker: str, value: float, d: datetime, price: Optional[float] = None) -> Trade:
        ...
        if price is None:
            ...
            raise ValueError(f"Input price: {price} is outside of bounds (Low: {data.loc[d, 'Low']} High: {data.loc[d, 'High']}) for date: {d.date().strftime('%Y-%m-%d')}")
...
if __name__ == "__main__":
    ...
    _ = broker.buy("AMZN", value=200., d=datetime(year=2023, month=2, day=6))
    trade = broker.close(broker.open_positions[0], d=datetime(year=2023, month=2, day=17))
    print(trade)